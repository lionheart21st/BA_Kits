...
def files_merge(shop_name_set):
    ...
    for name in shop_name_set:   #读取店名集合
        ...
        if os.path.exists(path+"/"+name+'广告销售.xlsx') :  #判断相应店名的广告销售Excel表格文件是否存在
            # print("ok")
            ...
            i+=1
        if os.path.exists(path+"/"+name+'销售总额.xlsx'):    #判断相应店名的销售总额Excel表格文件是否存在
            if os.path.exists(path+"/"+name)== False :    #如果店名目录不存在，则 以店名新建子目录
                os.mkdir(path + "/" + name)
            os.rename(path + "/" + name + '销售总额.xlsx',path + "/" + name + "/" + name + '销售总额.xlsx')  # 改变目标文件目录 即将目标文件放入相应的子目录中（其实就是在路径上加一层目录）
            df_log.loc[i] = [path + "/" + name, path + "/" + name + "/" + name + '销售总额.xlsx']  # 添加记录到《归并信息汇总.xlsx》
            i+=1
